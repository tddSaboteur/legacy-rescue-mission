...
public class JschSftpClient implements SftpClient {
    ...
    @Override
    public List<SftpFileMetadata> ls(String path) throws SftpClientException {
        try {
            return channel.ls(path).stream()
                    .map(SftpFileMetadata::fromLsEntry)
                    .toList();
        } catch (SftpException e) {
    ...
    public void init(JschSetup jschSetup) {
        ...
        Integer bulkRequests = jschSetup.sftpConfig().getBulkRequests();
        try {

            channelSetBulkRequests(bulkRequests);
    ...
    private void forceDisconnectSession() {
        if (session != null) {
            session.disconnect();
        }
        session = null;
